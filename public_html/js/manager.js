$(document).ready(function() {
    ...
    $('div#managerArea').on('click', 'button[data-flag]', function(event) {
        var self = $(this);
        var parent_form = $(self)
            .parents('form')
            .first();
        var flag_element = $(parent_form)
            .find('input[data-flag="' + $(self).data('flag') + '"]')
            .first();
    ...
    $('div#managerArea').on('click', 'button[data-action="cancel"],button[data-action="clear"]', function(event) {
        ...
        $(parent_form)
            ...
            .each(function(index, element) {
                ...
                if (has_visual) {
                    var visual_element = $(parent_form)
                        .find('span[data-visual="' + $(element).data('visual') + '"]')
                        .first();
                    visual_element.html($(element).data('original'));
                }
            });

        if (action == 'cancel') $(parent_form).removeClass('editing');
    });
...
function ticket_save_success(ajax) {
    if (ajax.success) {
        ...
        if (ajax.response.flags.ticket_update) {
            ...
            $(ticket_row)
                .parents('form.editable')
                .first()
                .replaceWith(ticket_body);

            // reselect our ticket row and then hook the ajax form
            var ticket_row = $('div.ticket[data-ticket="' + ticket_id + '"]');
            var parent_form = $(ticket_row)
                .parents('form.editable')
                .first();
