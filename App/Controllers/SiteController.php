...
class SiteController extends Controller
{
    ...
    public function login_get($in)
    {
        ...
        if ($ticket_code !== false) {
            // we found one, lets validate and redirect if its legit
    ...
    public function home_get($in)
    {
        ...
        if ($ticket_code === false) {
            ...
            $ticket = Ticket::select(array('id', 'code', 'username', 'master'))
                ->where('code', $ticket_code, '=')
                ->limit(1)
                ->get(true);
            if (!$ticket) {
        ...
        // player sync
        $player_sync = PlayerSync::select(array('*'))
            ->limit(1)
            ->get(true);
        ...
        $viewer = Viewer::select(array('viewerlist.id'))
            ->where('viewerlist.ticket', $ticket['id'])
            ->limit(1)
            ->get(true);
        if (!$viewer) {
            $viewer = new Viewer();
        ...
        // pull all viewers
        $viewers = Viewer::select(array('tickets.username'))
            ->join('tickets', 'viewerlist.ticket', 'tickets.id')
            ->where('viewerlist.ping', $ping_limit, '>=')
            ->orderBy('tickets.username', 'ASC')
            ->get(true);
        ...
        if (Request::isAjax()) {
            ...
            $messages = ChatMessage::raw(
                '
                    ...
                    Database::escape($last_message_id) .
                    '"
            ...
            // show chat only mode NOTE: THIS NEEDS TO BE REDONE CLEANLY, THIS IS COPY PASTED AND SLIGHTLY MODIFIED FROM ABOVE
    ...
    public function viewerlist_post()
    {
        ...
        if ($request->get('state') == 'watching') {
            ...
            // we are no longer watching. Delete this ( this is usually sent when the user leaves the page via the javascript function)
