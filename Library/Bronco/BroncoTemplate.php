...
class BroncoTemplate
{
    ...
    {
        $content_hash = md5(
            filemtime($this->template_path) . $this->template_path . json_encode($variables, JSON_FORCE_OBJECT)
        );
        $cache_file = self::$options['view_cache'] . DIRECTORY_SEPARATOR . $content_hash;

        if (BroncoCache::exist($cache_file)) {
            // we are cached we can go ahead and just simply
            $this->content = BroncoCache::read($cache_file);
        } else {
            // uncached, we need to process
    ...
    private function evaluate_expression($expression, &$variables)
    {
        ...
        if ($has_match && $has_expression) {
            // treat as explicit expression ( left side, operator, right side)
        ...
        } elseif (!$has_match && $has_expression) {
            // treat as implicit expression
    ...
    {
        $variable = null;
        $reserved_nonvariables = array('true', 'false', 'null');

        if (is_numeric($var_string)) {
            // this is a numeric and we should cast to float
            $variable = (float) $var_string;
        } elseif (in_array(strtolower($var_string), $reserved_nonvariables)) {
            ...
            (strpos($var_string, '"') === 0 && strrpos($var_string, '"') === 0) ||
            (strpos($var_string, "'") === 0 && strrpos($var_string, "'") === 0)
        ) {
    ...
    public function processConditions(&$variables)
    {
        ...
        foreach ($matches[1] as $original => $keys) {
            ...
            if ($result) {
                // look for a <true> block, if none is explicit then assume the entire block is provided only if true
                $expression_matches = array();
                $regex_option = preg_match('/<true(.*?)\>(.*?)\<\/true\>/s', $src_content, $expression_matches);

                $condition_content =
                    $regex_option && strlen($expression_matches[2]) ? $expression_matches[2] : $src_content;
                $condition_attributes =
                    $regex_option && strlen($expression_matches[1]) ? trim($expression_matches[1]) : array();
            } else {
                // look for a <false> element. If none is explicity set then go ahead and the entire block will be ignored
                $expression_matches = array();
                ...
                $condition_attributes =
                    $regex_match && strlen($expression_matches[1]) ? trim($expression_matches[1]) : array();
            }
    ...
    private function parseChildren(&$variables)
    {
        ...
        $matches = array();
        preg_match_all(
            "/(?:<template).*src\=['\"]([\/\a-zA-Z0-9\.\_]+?)['\"].*\/>|(?:<template).*src\=['\"]([\/\a-zA-Z0-9\.\_]+?)['\"].*\/?\>(?:.*<\/template\>)|(?:<template).*src\=['\"]([\/\a-zA-Z0-9\.\_]+?)['\"].*\/?\>(?:.*<\/template\>)/",
            $this->content,
            $matches
        );
    ...
    private function processInline(&$variables)
    {
        ...
        foreach ($matches[2] as $original => $match) {
            ...
            if ($attribute_to_set !== null && $attribute_value !== null) {
                if (strpos($filtered_element, $attribute_to_set) === false) {
                    // attribute does not exist. Regex to make it exist
                    $filtered_element = substr_replace(
                        $filtered_element,
                        ' ' . $attribute_to_set . '="' . $attribute_value . '">',
                        strrpos($filtered_element, '>', 0)
                    ); // insert into this position'
                } else {
                    // attribute does exist. Need to regex to modify it
                }
            }
    ...
    private function transform(&$variables, $key, &$found = false)
    {
        ...
        if ($total_levels == 1) {
            ...
            $variable = &$variables;
            $escape = false;
            while (($level = current($portions)) !== false && !$escape) {
                // cascade through our variables as needed
