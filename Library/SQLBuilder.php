...
class SQLBuilder
{
    ...
    public function select($column)
    {
        ...
        if (count($this->selects) == 1 && $this->selects[0] == '*') {
            unset($this->selects[0]);
        } //remove our default value
    ...
    public function get($rawArray = false)
    {
        ...
        foreach ($this->joins as $join) {
            $joinString .=
                $join['type'] . ' JOIN ' . $join['table'] . ' ON ' . $join['column1'] . ' = ' . $join['column2'];
        }
        ...
        foreach ($this->wheres as $where) {
            $assignmentType = isset($this->model->expected[$where['column']])
                ? $this->model->expected[$where['column']]
                : "'%s'";
            $whereString .=
                $index == 0
                    ? ' WHERE ' . $where['column'] . ' ' . $where['operator'] . ' ' . $assignmentType
                    : ' AND ' . $where['column'] . ' ' . $where['operator'] . ' ' . $assignmentType;
        ...
        $r = null;
        if ($this->isCounting && $results) {
            // if we are counting we only want the first row and first field back
            $row = $results->fetch_row();
            $r = $row[0];
        } elseif ($results) {
            // otherwise we go through and create the respective model for them
            $records = array();
    ...
    public function raw($query)
    {
        ...
        while ($row = $raw_results->fetch_assoc()) {
            // fetch results and store them off
            $query_items[] = $row;
        }
